...
# Tom Brady won this year his 6th SuperBowl, after 19 seasons in the NFL.
...
# Next let's plot Brady's passing yards per game by age:

plot(age, yards.per.game, type="l")
...
# sort Brady's season from most passing yards per game to least.
...
data <- read.csv("~/Desktop/STA 50/STA 50 - R/Data/receiving2018.csv")
...
# one reception in the 2018 NFL season.
...
yards.per.game.receiving = data$Yds / data$G
hist(yards.per.game.receiving[data$G>=15 & data$Pos == "RB"])
...
leaders = aggregate(data$Yds, list(Tm = data$Tm), max)
min(leaders$x)
...
# Hint: ?merge
data15 <- read.csv("~/Desktop/STA 50/STA 50 - R/Data/receiving2015.csv")
